...
const App: React.FC = () => {
  ...
  const LanguageSwitcher = () => (
    <div className="flex gap-2 bg-white/90 p-1 rounded-lg shadow-sm absolute bottom-4 left-4 z-[90] border border-gray-100">
  ...
  if (redirecting) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-brand-blue to-brand-purple flex items-center justify-center relative overflow-hidden">
  ...
  if (currentView === View.ADMIN_PANEL) {
    return <AdminGate language={language} onBack={() => setCurrentView(View.PARENT_DASHBOARD)} />;
  }
  if (currentView === View.ADMIN_TENANTS) {
    return <AdminTenants onBack={() => setCurrentView(View.PARENT_DASHBOARD)} language={language} />;
  }
